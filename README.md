...
| **addPermission( <br> string $entityId, <br> string $action, <br> string $resource, <br> bool $allowActionOnResource = true, <br> callable $additionalAssertions = null, <br> ...$argsForCallback <br>): self** | Used for adding a permission to an entity with the specified ID. If the entity does not exist, it will be created and the permission will be added to it. <br>See **__construct** in **[\SimpleAcl\Interfaces\PermissionInterface](src/interfaces/PermissionInterface.php)** for more information about all the parameters to this method (except the first). |
